...
function sub() {
  ...
  if (choice.value == "Choose Item" || n_choice.value == "Choose Quantity") {
    ...
    list.innerHTML += `<td>${name}</td> <td> = ${
      n_choice.value
    } Piece</td> <td> =>  ${choice.value * n_choice.value}</td>`;
  }
...
function add() {
  ...
  function addTotalCus() {
    ...
    totalSell.innerHTML =
      Number(totalSell.innerHTML) +
      Number(document.getElementById("total_value").innerHTML);
